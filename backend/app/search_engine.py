...
def _search_by_roles(db: Session, roles: List[str], limit: int = 50) -> List[models.Opportunity]:
    ...
    return get_base_active_query(db).filter(or_(*filters)).order_by(
        models.Opportunity.posted_date.desc().nulls_last(),
        models.Opportunity.id.desc()
    ).limit(limit).all()

def search_opportunities(db: Session, query: str, limit: int = 500) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """
    ...
    """
    base_q = get_base_active_query(db)
    total_db_active = base_q.count()

    if not query:
        # No query, return latest active jobs sorted by posted_date
        jobs = base_q.order_by(
            models.Opportunity.posted_date.desc().nulls_last(),
            models.Opportunity.id.desc()
        ).limit(limit).all()
        return [{"opportunity": j, "search_level": 1} for j in jobs], {"exact": len(jobs), "related": 0, "total_db_active": total_db_active}
